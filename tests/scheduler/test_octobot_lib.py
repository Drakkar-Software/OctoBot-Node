...
@pytest.fixture
def multiple_actions_bundle_no_wait(market_order_action, limit_order_action):
    all = {
        "params": {
    ...
    all["params"]["ACTIONS"] = "deposit,trade,withdraw"
    return all
...
class TestOctoBotActionsJob:
    ...
    async def test_run_market_order_action(self, market_order_action):
        ...
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "apply_configuration"
        ...
        assert next_actions_description.immediate_actions[0].config["content"]["SIGNAL"] == "BUY"
        job2 = octobot_lib.OctoBotActionsJob(
            next_actions_description.to_dict(include_default_values=False)
        )
        result = await job2.run()
        assert len(result.processed_actions) == 1
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "BUY"
        assert len(result.get_created_orders()) == 1
        assert result.get_created_orders()[0]["symbol"] == "ETH/BTC"
        assert result.get_created_orders()[0]["amount"] == 1
        assert result.get_created_orders()[0]["type"] == "market"
        assert result.get_created_orders()[0]["side"] == "buy"
    ...
    async def test_run_limit_order_action(self, limit_order_action): #TODO
        # step 1: configure the task
        ...
        assert result.get_created_orders()[0]["symbol"] == "ETH/BTC"
        assert result.get_created_orders()[0]["amount"] == 1
        assert result.get_created_orders()[0]["type"] == "limit"
        assert result.get_created_orders()[0]["side"] == "buy"
        assert result.next_actions_description is None # no more actions to execute

    async def test_polymarket_trade_action(self, polymarket_trade_action): #TODO
        # step 1: configure the task
        job = octobot_lib.OctoBotActionsJob(polymarket_trade_action)
        result = await job.run()
        assert len(result.processed_actions) == 1
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "apply_configuration"
        pre_trade_portfolio = job.after_execution_state.bots[0].exchange_account_elements.portfolio.content
        ...
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "BUY"
        assert len(result.get_created_orders()) == 1
        assert result.get_created_orders()[0]["symbol"] == "what-price-will-bitcoin-hit-in-january-2026/USDC:USDC-260131-0-YES"
        assert result.get_created_orders()[0]["amount"] == 1
        assert result.get_created_orders()[0]["type"] == "market"
        assert result.get_created_orders()[0]["side"] == "buy"
    ...
    async def test_run_deposit_action(self, deposit_action):
        ...
        assert next_actions_description is not None
        assert len(next_actions_description.immediate_actions) == 1
        assert next_actions_description.immediate_actions[0].config["content"]["SIGNAL"] == "transfer_funds"
        job2 = octobot_lib.OctoBotActionsJob(
            next_actions_description.to_dict(include_default_values=False)
        )
        result = await job2.run()
        assert len(result.processed_actions) == 1
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "transfer_funds"
        assert result.next_actions_description is None # no more actions to execute
    ...
    async def test_run_withdraw_action(self, withdraw_action):
        ...
        assert next_actions_description is not None
        assert len(next_actions_description.immediate_actions) == 1
        assert next_actions_description.immediate_actions[0].config["content"]["SIGNAL"] == "withdraw_funds"
        job2 = octobot_lib.OctoBotActionsJob(
            next_actions_description.to_dict(include_default_values=False)
        )
        result = await job2.run()
        assert len(result.processed_actions) == 1
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "withdraw_funds"
        assert result.next_actions_description is None # no more actions to execute
    ...
    async def test_run_multiple_actions_bundle_no_wait(self, multiple_actions_bundle_no_wait):
        ...
        next_actions_description = result.next_actions_description
        assert next_actions_description is not None
        assert len(next_actions_description.immediate_actions) == 3
        assert next_actions_description.immediate_actions[0].config["content"]["SIGNAL"] == "transfer_funds"
        assert next_actions_description.immediate_actions[1].config["content"]["SIGNAL"] == "BUY"
        assert next_actions_description.immediate_actions[2].config["content"]["SIGNAL"] == "withdraw_funds"
        job2 = octobot_lib.OctoBotActionsJob(
            next_actions_description.to_dict(include_default_values=False)
        )
        result = await job2.run()
        assert len(result.processed_actions) == 3
        assert result.processed_actions[0].config["content"]["SIGNAL"] == "transfer_funds"
        assert result.processed_actions[0].result["amount"] == 1
        assert result.processed_actions[1].config["content"]["SIGNAL"] == "BUY"
        assert result.processed_actions[2].config["content"]["SIGNAL"] == "withdraw_funds"
        assert result.processed_actions[2].result["amount"] == 0
        assert len(result.get_created_orders()) == 1
        assert result.get_created_orders()[0]["symbol"] == "ETH/BTC"
        assert result.get_created_orders()[0]["amount"] == 1
        assert result.get_created_orders()[0]["type"] == "market"
        ...
        # ensure deposit is successful
        post_deposit_portfolio = job2.after_execution_state.bots[0].exchange_account_elements.portfolio.content
        assert pre_trade_portfolio["BTC"][common_constants.PORTFOLIO_AVAILABLE] < post_deposit_portfolio["BTC"][common_constants.PORTFOLIO_AVAILABLE] < pre_trade_portfolio["BTC"][common_constants.PORTFOLIO_AVAILABLE] + 1
        # + 1 for 1 BTC deposit
        assert pre_trade_portfolio["BTC"][common_constants.PORTFOLIO_TOTAL] < post_deposit_portfolio["BTC"][common_constants.PORTFOLIO_TOTAL] < pre_trade_portfolio["BTC"][common_constants.PORTFOLIO_TOTAL] + 1
        # + 1 for 1 BTC deposit

        # bought ETH - fees (0 were withdrawn because buy order did not execute at withdraw time)
        assert post_deposit_portfolio["ETH"][common_constants.PORTFOLIO_AVAILABLE] == 0.999
        assert post_deposit_portfolio["ETH"][common_constants.PORTFOLIO_TOTAL] == 0.999
    ...
    async def test_run_multiple_actions_bundle_with_wait(self, multiple_action_bundle_with_wait):
        ...
        assert result.next_actions_description.immediate_actions[0].config["content"]["SIGNAL"] == "BUY"
        post_deposit_portfolio = job2.after_execution_state.bots[0].exchange_account_elements.portfolio.content
        assert post_deposit_portfolio["BTC"] == {
            common_constants.PORTFOLIO_AVAILABLE: 2,
            common_constants.PORTFOLIO_TOTAL: 2,
