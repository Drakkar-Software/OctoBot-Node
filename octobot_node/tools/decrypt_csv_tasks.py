...
This script takes an encrypted CSV file as input, decrypts the content column using the provided
...
def decrypt_csv_file_tool(
    ...
    """Decrypt a CSV file and write the decrypted version to output file.
    
    Args:
        input_file_path: Path to the input encrypted CSV file
    ...
    print(f"Decrypting CSV file: {input_file_path}")
    decrypt_csv_file(
...
def decrypt_csv_file_from_keys_file(
    ...
    """Decrypt a CSV file using keys from a JSON keys file.
    
    This is a convenience function that extracts RSA private key and ECDSA public key
    from a JSON keys file and decrypts the CSV.
...
Examples:
  ...
  %(prog)s encrypted_tasks.csv  # Uses TASKS_INPUTS_RSA_PRIVATE_KEY and TASKS_INPUTS_ECDSA_PUBLIC_KEY environment variables
    ...
    parser.add_argument(
        "csv_file",
        type=str,
        help="Path to the input encrypted CSV file to decrypt"
    )
